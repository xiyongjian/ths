...
public class Download {
	...
	public void addResult(String id, String r, String stat) {
		...
		try {
			if (_results.containsKey(id)) {
				p("work " + id + " already done and set result");
				return;
			}
			_results.put(id, r);
			_stats.put(id, stat);
			p("result " + id + " added, remain tasks : " + (this._totalTasks - _results.size()));
		} finally {
			_resultLock.unlock();
		}
	}
		...
		public Callable<String> createWorker() {
			Callable<String> worker = new Callable<String>() {
				public String call() throws Exception {
					...
					p("worker " + _id + " running, thread id " + Thread.currentThread().getId()
							+ ", download code " + code);
					String strResult = JDIBridge.THS_RealtimeQuotes(code, _columns);
					p("worker " + _id + " return length : " + strResult.length());
	...
	public void run() throws DownloadException {
		...
		System.out.println("group size " + _group_size + ", start at " + date_str);
		...
		p("total todo tasks : " + this._totalTasks);
				...
				try {
					...
					for (Future<String> future : futList) {
						...
						if (future.isDone()) {
							...
							p("worker " + id + " done, removed from waitList and idTasks list");
					...
					if (this.isResultReady()) {
						p("result ready, done, break from inner loop");
						break;
					}
		...
		p("write all result to file : " + file);
		FileWriter fw = null;
		try {
	...
	public static Download createDownload(String prefix, List<String> codes, String columns, int group_size,
			int timeout_ms) {
		...
		p(String.format("Downoad %s, group_size %d, timeout %d\ncodes : %s\ncolumns : %s",
